...
class iworks_aqualog_posttype_post extends iworks_aqualog_posttype_base {
	...
	public function get_related( $content, $post_ID ) {
		...
		$content .= sprintf( '<p class="related-posts-title">%s</p>', esc_html__( 'Related posts:', 'aqualog' ) );
	...
	public function html_post_gallery( $post ) {
		...
		printf(
			...
			esc_html__( 'Add picture', 'aqualog' )
		);
...
<div class="image-wrapper<?php echo esc_attr( empty( $src ) ? '' : ( 0 < $value ? ' has-file' : ' has-old-file' ) ); ?>">
	...
	<button type="button" class="image-reset" aria-label="<?php esc_attr_e( 'Remove file', 'aqualog' ); ?>"><span class="dashicons dashicons-trash"></span></button>
