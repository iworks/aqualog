...
class iworks_aqualog_posttype_promo extends iworks_aqualog_posttype_base {
	...
	public function __construct() {
		...
		$this->meta_boxes[ $this->posttypes_names[ $this->posttype_name ] ] = array(
			'featured-url' => array(
				'title'  => __( 'URL Configuration', 'aqualog' ),
				'fields' => array(
					array(
						'name'  => 'target_button_url',
						'type'  => 'url',
						'label' => esc_html__( 'Target Button URL', 'aqualog' ),
					),
					array(
						'name'  => 'target_button_text',
						'label' => esc_html__( 'Target Button Text', 'aqualog' ),
					),
				),
			),
		);
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) {
				...
				if ( empty( $button_label_more ) ) {
					$button_label_more = _x( 'Find out more', 'Promo button text', 'aqualog' );
				}
				$content .= sprintf(
	...
	public function action_init_register_post_type() {
		$labels = array(
			'name'                  => _x( 'Featured', 'Post Type General Name', 'aqualog' ),
			'singular_name'         => _x( 'Featured', 'Post Type Singular Name', 'aqualog' ),
			'menu_name'             => __( 'Featured', 'aqualog' ),
			'name_admin_bar'        => __( 'Featured', 'aqualog' ),
			'archives'              => __( 'Featured', 'aqualog' ),
			'all_items'             => __( 'Featured', 'aqualog' ),
			'add_new_item'          => __( 'Add New Featured', 'aqualog' ),
			'add_new'               => __( 'Add New', 'aqualog' ),
			'new_item'              => __( 'New Featured', 'aqualog' ),
			'edit_item'             => __( 'Edit Featured', 'aqualog' ),
			'update_item'           => __( 'Update Featured', 'aqualog' ),
			'view_item'             => __( 'View Featured', 'aqualog' ),
			'view_items'            => __( 'View Featured', 'aqualog' ),
			'search_items'          => __( 'Search Featured', 'aqualog' ),
			'not_found'             => __( 'Not found', 'aqualog' ),
			'not_found_in_trash'    => __( 'Not found in Trash', 'aqualog' ),
			'items_list'            => __( 'Featured list', 'aqualog' ),
			'items_list_navigation' => __( 'Featured list navigation', 'aqualog' ),
			'filter_items_list'     => __( 'Filter items list', 'aqualog' ),
		);
		$args   = array(
			'can_export'          => true,
			'capability_type'     => 'page',
			'description'         => __( 'Featured', 'aqualog' ),
			'exclude_from_search' => true,
			'has_archive'         => false,
			'hierarchical'        => false,
			'label'               => __( 'Featured', 'aqualog' ),
			'labels'              => $labels,
	...
	public function html_url( $post ) {
		...
		echo '<label><h4>';
		_e( 'Target Button Label', 'aqualog' );
		echo '</h4>';
		printf(
		...
		printf(
			...
			__( 'Leave empty to default', 'aqualog' )
		);
		/**
		 * is ukrainian text?
		 */
