...
class iworks_aqualog_posttype_project extends iworks_aqualog_posttype_base {
	...
	public function action_init_settings() {
		...
		$this->meta_boxes[ $this->posttypes_names[ $this->posttype_name ] ] = array(
			'project-data'  => array(
				'title'  => __( 'Project Data', 'aqualog' ),
				'fields' => array(
					array(
						'name'  => 'icon',
						'type'  => 'image',
						'label' => esc_html__( 'Icon', 'aqualog' ),
					),
					array(
						'name'  => 'opinion_url',
						'type'  => 'url',
			...
			'project-media' => array(
				'title'  => __( 'Project Media', 'aqualog' ),
				'fields' => array(
					array(
						'name'  => 'icon',
						'type'  => 'image',
	...
	public function setup() {
		$this->partners_types = array(
			'lider'         => __( 'Liders', 'aqualog' ),
			'scientific'    => __( 'Scientific Partners', 'aqualog' ),
			'business'      => __( 'Business Partners', 'aqualog' ),
			'partner'       => __( 'Partners', 'aqualog' ),
			'subcontractor' => __( 'Subcontractors', 'aqualog' ),
		);
	}
	...
	public function get_random( $content, $posts_per_page ) {
		...
		if ( 'pl_PL' === get_locale() ) {
			...
			$content .= sprintf( '<span class="section-title">%s</span>', esc_html__( 'Projects of OPI PIB', 'aqualog' ) );
		}
	...
	public function get_list( $content ) {
		...
		if ( $url ) {
			$content .= sprintf(
				...
				$url,
				esc_html__( 'Browse all projects', 'aqualog' )
			);
		}
		return $content;
	}
		...
		foreach ( $this->fields as $key => $one ) {
			...
			if ( isset( $one['type'] ) && 'url' === $one['type'] ) {
				$value = sprintf(
					...
					esc_url( $value ),
					esc_attr__( 'Opens in a new window', 'aqualog' )
				);
	...
	public function action_init_register_post_type() {
		$labels = array(
			'name'                  => _x( 'Projects', 'Post Type General Name', 'aqualog' ),
			'singular_name'         => _x( 'Project', 'Post Type Singular Name', 'aqualog' ),
			'menu_name'             => __( 'Projects', 'aqualog' ),
			'name_admin_bar'        => __( 'Projects', 'aqualog' ),
			'archives'              => __( 'Projects', 'aqualog' ),
			'all_items'             => __( 'Projects', 'aqualog' ),
			'add_new_item'          => __( 'Add New project', 'aqualog' ),
			'add_new'               => __( 'Add New', 'aqualog' ),
			'new_item'              => __( 'New project', 'aqualog' ),
			'edit_item'             => __( 'Edit project', 'aqualog' ),
			'update_item'           => __( 'Update project', 'aqualog' ),
			'view_item'             => __( 'View project', 'aqualog' ),
			'view_items'            => __( 'View project', 'aqualog' ),
			'search_items'          => __( 'Search project', 'aqualog' ),
			'not_found'             => __( 'Not found', 'aqualog' ),
			'not_found_in_trash'    => __( 'Not found in Trash', 'aqualog' ),
			'items_list'            => __( 'Project list', 'aqualog' ),
			'items_list_navigation' => __( 'Project list navigation', 'aqualog' ),
			'filter_items_list'     => __( 'Filter items list', 'aqualog' ),
		);
		$args   = array(
			'can_export'          => true,
			'capability_type'     => 'page',
			'description'         => __( 'Project', 'aqualog' ),
			'exclude_from_search' => true,
			'has_archive'         => true,
			'hierarchical'        => false,
			'label'               => __( 'Projects', 'aqualog' ),
			'labels'              => $labels,
			'public'              => true,
			...
			'rewrite'             => array(
				'slug' => _x( 'Project', 'slug for single project', 'aqualog' ),
			),
		);
	...
	private function html_partners( $post, $type ) {
		...
		printf(
			...
			esc_html__( 'Add a partner', 'aqualog' )
		);
		echo '</p>';
		printf(
	...
	private function set_fields() {
		$this->fields = array(
			'_project_date_start'     => array(
				'label' => __( 'Project start date', 'aqualog' ),
				'type'  => 'date',
			),
			'_project_date_end'       => array(
				'label' => __( 'Project end date', 'aqualog' ),
				'type'  => 'date',
			),
			'_realization_date_start' => array(
				'label' => __( 'Realization start date', 'aqualog' ),
				'type'  => 'date',
			),
			'_realization_date_end'   => array(
				'label' => __( 'Realization end date', 'aqualog' ),
				'type'  => 'date',
			),
			'_project_cost'           => array(
				'label'    => __( 'Project cost', 'aqualog' ),
				'type'     => 'number',
				'sanitize' => 'floatval',
				'sufix'    => __( 'PLN', 'aqualog' ),
			),
			'_project_funding'        => array(
				'label'    => __( 'Project amount of funding', 'aqualog' ),
				'type'     => 'number',
				'sanitize' => 'floatval',
				'sufix'    => __( 'PLN', 'aqualog' ),
			),
			'_project_currency'       => array(
				'label'    => __( 'Project currency', 'aqualog' ),
				'type'     => 'text',
				'sanitize' => 'esc_html',
				'hide'     => true,
			),
			'_project_url'            => array(
				'label'    => __( 'Project URL', 'aqualog' ),
				'type'     => 'url',
				'sanitize' => 'esc_url',
			),
		);
	...
	protected function partner_row( $data = array(), $type = '' ) {
		...
		printf(
			...
			esc_attr__( 'Remove Partner', 'aqualog' )
		);
		echo '</div>';
	}
